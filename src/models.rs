...
impl ToString for ConnectFourMove {
    fn to_string(&self) -> String {
        match *self {
            ConnectFourMove::XPosition => "  X  ".red().bold().to_string(),
            ConnectFourMove::XIgnoredPosition => " .x. ".red().bold().to_string(),
            ConnectFourMove::OPosition => "  O  ".blue().bold().to_string(),
            ConnectFourMove::OIgnoredPosition => " .o. ".blue().bold().to_string(),
            ConnectFourMove::UnplayablePosition => "#####".white().bold().to_string(),
...
impl ConnectFourBoard {
    pub fn new(column_sizes: &Vec<u32>, ignored_positions: &Vec<Position>) -> ConnectFourBoard {
        ...
        let board = column_sizes
            .iter()
            .enumerate()
            .map(|(x, xth_column_size)| {
                (0..highest_column)
                    .map(move |y| {
                        // move resolves the borrowing problem by moving the lifecycle of x and xth_column_size to this scope
                        if (y as u32) < (highest_column - xth_column_size) {
                            // position x is higher than column actual size
                            ConnectFourMove::UnplayablePosition
                        } else {
                            ignored_positions
                                .iter()
                                .find(|ignored_position| {
                                    ignored_position.row == (highest_column - y as u32)
                                        && ignored_position.column == x as u32
                                })
    ...
    pub fn create_another(
        board: Vec<Vec<ConnectFourMove>>,
        height: usize,
        width: usize,
    ) -> ConnectFourBoard {
        ConnectFourBoard {
    ...
    pub fn make_move(&mut self, move_type: ConnectFourMove, move_column: usize) -> bool {
        if move_column >= self.width {
            return false;
        }

        let column = &mut self.board[move_column];
        let empty_position_opt = column.iter_mut().rev().find(|position| {
            matches!(
                **position,
                ConnectFourMove::EmptyPosition | ConnectFourMove::EmptyIgnoredPosition
            )
        });
        let Some(empty_position) = empty_position_opt else {
            return false;
        };

        *empty_position = match *empty_position {
            ConnectFourMove::EmptyIgnoredPosition if move_type == ConnectFourMove::XPosition => {
                ConnectFourMove::XIgnoredPosition
            }
            ConnectFourMove::EmptyIgnoredPosition if move_type == ConnectFourMove::OPosition => {
                ConnectFourMove::OIgnoredPosition
            }
        ...
        self.board
            ...
            .map(|column| {
                column
                    .iter()
                    .filter(|element| {
                        element == &&ConnectFourMove::EmptyPosition
                            || element == &&ConnectFourMove::EmptyIgnoredPosition
                    })
                    .count()
            })
            .collect::<Vec<_>>()
    ...
    fn valid_position(&self, cell: ConnectFourMove) -> bool {
        match cell {
            ConnectFourMove::XPosition | ConnectFourMove::OPosition => true,
            _ => false,
        }
    }
        ...
        if !self.valid_position(candidate) {
            ...
            for index in 1..4 {
                horizontal_win = horizontal_win
                    && x + upper_limit < self.width
                    && self.board[x + index][y] == candidate;
                vertical_win = vertical_win
                    && y + upper_limit < self.height
                    && candidate == self.board[x][y + index];
                diagonal_up_win = diagonal_up_win
                    && (y + upper_limit) < self.height
                    && (x + upper_limit) < self.width
                    && candidate == self.board[x + index][y + index];
                diagonal_down_win = diagonal_down_win
                    && (y as i32 - index as i32) >= 0
                    && x + index < self.width
                    && candidate == self.board[x + index][y - index];
            }
            ...
            match (
                ...
                (_, _, true, _) => {
                    Some(vec![(y, x), (y + 1, x + 1), (y + 2, x + 2), (y + 3, x + 3)])
                }
                (_, _, _, true) => {
                    Some(vec![(y, x), (y - 1, x + 1), (y - 2, x + 2), (y - 3, x + 3)])
                }
    ...
    pub fn return_winner(
        &self,
        y: usize,
        x: usize,
    ) -> Option<(ConnectFourMove, Vec<(usize, usize)>)> {
    ...
    fn winning_move(
        &self,
        index: usize,
        player_move: ConnectFourMove,
        free_moves: &Vec<usize>,
    ) -> Option<usize> {
        let mut cloned = self.clone();
        //println!("FAOFAFA {:?} {}", free_moves, free_moves[index]);
        if !cloned.make_move(player_move, free_moves[index]) {
            return if index == 0 {
                None
            } else {
                self.winning_move(index - 1, player_move, &free_moves)
            };
        }
    ...
    pub fn next_winning_move(&self, player: Player) -> Option<usize> {
        ...
        self.winning_move(
            (free_moves.len() as u32 - 1u32) as usize,
            player_move,
            &free_moves,
        )
    }
    ...
    pub fn free_moves(&self) -> Vec<usize> {
        self.empty_moves_by_column()
            .iter()
            .enumerate()
            .filter(|(_index, amount_left)| **amount_left != 0usize)
            .map(|(index, _)| index)
            .collect::<Vec<_>>()
    }
...
static ASCII_UPPERCASE: &'static [char] = &[
        ...
        let body: String = (0..self.height)
            .map(|y| {
                ...
                let line = (0..self.width)
                    .map(|x| self.board[x as usize][y as usize].to_string())
                    .collect::<Vec<_>>()
                    .join("|");
                base.push_str(&line);
                base.push_str("|");
...
mod tests {
    ...
    fn check_winning_position() {
        ...
        assert!(board.is_winning_position(0usize, 0usize).is_none());
        ...
        print!("board status:\n {}", board.to_string());
        assert!(board.is_winning_position(5usize, 1usize).is_some());
    }

    #[test]
    fn rejects_full_column() {
        let mut board = ConnectFourBoard::new(&vec![1u32], &vec![]);
        assert!(board.make_move(ConnectFourMove::OPosition, 0));
        assert!(!board.make_move(ConnectFourMove::XPosition, 0));
    }
}
